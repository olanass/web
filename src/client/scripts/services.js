...
async function fetchMyServices() {
  ...
  try {
    ...
    tbody.innerHTML = data.services.map(service => `<tr><td><strong>${escapeServiceHtml(service.name)}</strong><div class="project-description">${escapeServiceHtml(service.description)}</div></td><td class="project-value"><strong>${escapeServiceHtml(service.billingMode === 'metered' ? 'Usage-based' : service.price)} ${escapeServiceHtml(service.currency)}</strong></td><td class="project-value">${Number(service.requests || 0).toLocaleString()}</td><td class="project-value text-success"><strong>${escapeServiceHtml(service.revenue)} ${escapeServiceHtml(service.currency)}</strong></td><td><span class="status-live-pill" data-status="${escapeServiceHtml(service.status)}">${escapeServiceHtml(service.status)}</span></td><td><div class="project-actions"><a href="/services/${encodeURIComponent(service.slug)}" class="btn btn-secondary btn-sm">View ↗</a></div></td></tr>`).join('');
    [...tbody.querySelectorAll('tr')].forEach((row, index) => {
      const service = data.services[index];
      const cell = row.querySelector('.project-actions');
